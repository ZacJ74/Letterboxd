...
const authForm = (req, res) => {
    if (req.session.userId) {
        return res.redirect('/'); // Already logged in → dashboard
    }
    res.render('auth-form', { title: 'Login / Sign Up' });
};
...
const logout = (req, res) => {
    req.session.destroy(err => {
        if (err) console.error(err);
        res.redirect('/users/auth'); // After logout → back to login/signup
    });
};
